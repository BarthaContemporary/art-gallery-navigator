-- 0043_consignment_share_costs
-- Rework jvb_share_gbp across the stock-book views: purchase cost still
-- drives the margin VAT but is excluded from the shared pool; restoration /
-- other (and margin-scheme import VAT) are carried by J.v.d.B. out of its
-- share. Regenerated from the live definitions, swapping only that column.
...
SELECT p.id AS piece_id,
   ...
   FROM pieces p
     ...
     LEFT JOIN LATERAL ( SELECT
                CASE WHEN p.consignment_share_pct IS NULL THEN NULL::numeric ELSE round(p.consignment_share_pct / 100::numeric * GREATEST(COALESCE(f.sold_price_gbp, 0::numeric) - vd.vat_due_gbp, 0::numeric) - (greatest(coalesce(f.total_cost_gbp,0::numeric) - coalesce(f.purchase_cost_gbp,0::numeric), 0::numeric) + iv.import_vat_cost), 2) END AS jvb_share_gbp) sh ON true
...
SELECT p.id AS piece_id,
   ...
   FROM pieces p
     ...
     LEFT JOIN LATERAL ( SELECT s.shipment_date,
         ...
         LIMIT 1) tmp ON true
     LEFT JOIN LATERAL ( SELECT
                CASE WHEN p.consignment_share_pct IS NULL THEN NULL::numeric ELSE round(p.consignment_share_pct / 100::numeric * GREATEST(COALESCE(f.sold_price_gbp, 0::numeric) - 0::numeric, 0::numeric) - greatest(coalesce(f.total_cost_gbp,0::numeric) - coalesce(f.purchase_cost_gbp,0::numeric), 0::numeric), 2) END AS jvb_share_gbp) sh ON true
