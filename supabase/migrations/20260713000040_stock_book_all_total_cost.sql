-- 0040_stock_book_all_total_cost
-- Expose total_cost_gbp on the combined stock-book view so the per-work preview
-- panel can show an exact "sold − total cost = margin" working (total cost
-- folds in restoration / other costs, not just the purchase price). CREATE OR
-- REPLACE can only append, so total_cost_gbp lands as the final column.
