-- 0045_consignment_share_reimburse
-- Correct jvb_share_gbp: restoration/other (and margin import VAT) are a
-- shared expense off the profit pool, reimbursed to J.v.d.B.. So the figure
-- is what J.v.d.B. receives = share% of (sold - VAT - costs) + costs.
...
SELECT p.id AS piece_id,
   ...
   FROM pieces p
     ...
     LEFT JOIN LATERAL ( SELECT
                CASE WHEN p.consignment_share_pct IS NULL THEN NULL::numeric ELSE round(p.consignment_share_pct / 100::numeric * GREATEST(COALESCE(f.sold_price_gbp, 0::numeric) - vd.vat_due_gbp - (greatest(coalesce(f.total_cost_gbp,0::numeric) - coalesce(f.purchase_cost_gbp,0::numeric), 0::numeric) + iv.import_vat_cost), 0::numeric) + (greatest(coalesce(f.total_cost_gbp,0::numeric) - coalesce(f.purchase_cost_gbp,0::numeric), 0::numeric) + iv.import_vat_cost), 2) END AS jvb_share_gbp) sh ON true
...
SELECT p.id AS piece_id,
   ...
   FROM pieces p
     ...
     LEFT JOIN LATERAL ( SELECT
                ...
                END AS vat_due_gbp) vd ON true
     LEFT JOIN LATERAL ( SELECT
                CASE WHEN p.consignment_share_pct IS NULL THEN NULL::numeric ELSE round(p.consignment_share_pct / 100::numeric * GREATEST(COALESCE(f.sold_price_gbp, 0::numeric) - vd.vat_due_gbp - greatest(coalesce(f.total_cost_gbp,0::numeric) - coalesce(f.purchase_cost_gbp,0::numeric), 0::numeric), 0::numeric) + greatest(coalesce(f.total_cost_gbp,0::numeric) - coalesce(f.purchase_cost_gbp,0::numeric), 0::numeric), 2) END AS jvb_share_gbp) sh ON true
...
SELECT p.id AS piece_id,
   ...
   FROM pieces p
     ...
     LEFT JOIN LATERAL ( SELECT s.shipment_date,
         ...
         LIMIT 1) tmp ON true
     LEFT JOIN LATERAL ( SELECT
                CASE WHEN p.consignment_share_pct IS NULL THEN NULL::numeric ELSE round(p.consignment_share_pct / 100::numeric * GREATEST(COALESCE(f.sold_price_gbp, 0::numeric) - 0::numeric - greatest(coalesce(f.total_cost_gbp,0::numeric) - coalesce(f.purchase_cost_gbp,0::numeric), 0::numeric), 0::numeric) + greatest(coalesce(f.total_cost_gbp,0::numeric) - coalesce(f.purchase_cost_gbp,0::numeric), 0::numeric), 2) END AS jvb_share_gbp) sh ON true
