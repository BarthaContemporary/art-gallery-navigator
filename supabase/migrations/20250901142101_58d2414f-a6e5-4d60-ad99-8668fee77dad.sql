...
BEGIN
  ...
  SELECT 
    ...
    asc.user_id,
    asc.bucket_name,
    decrypt_credential(asc.access_key) as access_key,
    decrypt_credential(asc.secret_key) as secret_key,
    asc.endpoint_url,
    asc.region,
    asc.is_active,
    asc.created_at,
    asc.updated_at
  FROM public.admin_storage_credentials asc
  WHERE asc.user_id = auth.uid();
...
BEGIN
  ...
  SELECT 
    ...
    asc.created_at,
    asc.updated_at
  FROM public.artist_storage_credentials asc
  WHERE asc.artist_id = target_artist_id;
...
BEGIN
  ...
  SELECT 
    ...
    ssc.name,
    ssc.bucket_name,
    decrypt_credential(ssc.access_key) as access_key,
    decrypt_credential(ssc.secret_key) as secret_key,
    ssc.endpoint_url,
    ssc.region,
    ssc.is_active,
    ssc.created_at,
    ssc.updated_at
  FROM public.shared_storage_credentials ssc
