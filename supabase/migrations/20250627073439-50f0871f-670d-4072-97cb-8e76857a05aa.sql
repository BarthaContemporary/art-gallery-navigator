...
-- Fix the WebDAV folder access functions to work with existing file sharing system

-- Update get_user_accessible_folders to return actual folders from the system
...
-- Update get_user_accessible_documents to work with the folder structure
...
BEGIN
  ...
  ELSE
    -- Regular users can only access documents they have permission for
    RETURN QUERY
    SELECT 
      d.id as document_id,
      d.file_name as document_name,
    ...
    WHERE d.is_deleted = false
      ...
      AND (
        ...
        (d.folder_id IS NOT NULL AND (fa.user_id = current_user_id OR f.created_by = current_user_id)) OR
        d.artist_id IS NULL -- Allow access to documents not tied to specific artists
      )
    ORDER BY d.file_name;
  END IF;
END;
