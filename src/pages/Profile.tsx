...
export default function Profile() {
  ...
  if (!user) {
    return <div className="p-4 sm:p-6 max-w-7xl mx-auto text-center">
        <p>Loading profile...</p>
      </div>;
  }
  return <div className="p-4 sm:p-6 max-w-7xl mx-auto">
      <Card className="w-full max-w-md mx-auto p-4 shadow-md bg-transparent">
        <CardHeader className="flex flex-col items-center bg-white">
          <div className="relative group">
            ...
            <Button variant="outline" size="icon" className="absolute bottom-2 right-2 h-8 w-8 rounded-full bg-background opacity-0 group-hover:opacity-100 transition-opacity" onClick={triggerFileInput} disabled={isUploading} aria-label="Upload new avatar">
              {isUploading ? <Loader2 className="h-4 w-4 animate-spin" /> : <UploadCloud className="h-4 w-4" />}
            </Button>
            <Input type="file" ref={fileInputRef} onChange={handleAvatarChange} className="hidden" accept="image/png, image/jpeg, image/gif" disabled={isUploading} />
          </div>
        ...
        <CardContent className="space-y-4 mt-4">
          ...
          <div className="border rounded-md p-3">
            ...
            {isEditingFullName ? <div className="flex items-center gap-2 mt-1">
                <Input value={fullName} onChange={e => setFullName(e.target.value)} placeholder="Enter your full name" className="flex-1" disabled={isUpdatingFullName} />
                <Button size="icon" variant="ghost" onClick={handleUpdateFullName} disabled={isUpdatingFullName || !fullName.trim()} className="h-8 w-8">
                  {isUpdatingFullName ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}
                </Button>
                <Button size="icon" variant="ghost" onClick={handleCancelEdit} disabled={isUpdatingFullName} className="h-8 w-8">
                  <X className="h-4 w-4" />
                </Button>
                ...
                <Button size="icon" variant="ghost" onClick={() => setIsEditingFullName(true)} className="h-8 w-8">
          ...
          {isArtist && currentUserArtist && <div className="border rounded-md p-3">
              ...
              {currentUserArtist.image_url && <p className="text-xs text-muted-foreground mt-1">
