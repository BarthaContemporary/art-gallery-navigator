...
const Dashboard = () => {
  ...
  return <div className="p-4 sm:p-6 max-w-7xl mx-auto">
      ...
      {/* Recent Activity and Inventory Status moved up */}
      <div className="grid gap-4 grid-cols-1 lg:grid-cols-2 mt-0 mb-8"> {/* Changed mt-8 to mt-0 and added mb-8 */}
        <Card>
          <CardHeader>
          ...
          <CardContent>
            {isLoading ? <div className="space-y-3">
                {[1, 2, 3].map(n => <div key={n} className="flex items-center gap-4">
                ...
                {stats?.recent_activities && stats.recent_activities.length > 0 ? stats.recent_activities.map((activity, index) => <div key={index} className="flex items-center gap-4">
                    ...
                    </div>) : <div className="text-muted-foreground text-sm">No recent activities found.</div>}
              </div>}
          </CardContent>
        </Card>
      ...
      {isAdmin && <div className="mt-8 mb-8"> {/* Added mb-8 for spacing */}
          {/* Removed h2 title as it was empty */}
