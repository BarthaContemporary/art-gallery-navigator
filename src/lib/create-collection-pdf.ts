...
export async function createCollectionPDF(
  ...
  try {
    ...
    // Important: set the background color to white for proper rendering
    tempDiv.style.backgroundColor = 'white';
    tempDiv.innerHTML = htmlContent;
    document.body.appendChild(tempDiv);
    
    // Pre-load the stationery image to ensure it's available for PDF generation
    if (useStationery) {
    ...
    const canvas = await html2canvas(tempDiv, {
      scale: 4.0, // Higher quality rendering
      useCORS: true,
      logging: false,
      allowTaint: true,
